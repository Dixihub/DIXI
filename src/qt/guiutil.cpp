...
{
    // Convert DIXI:// to DIXI:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("DIXI://"))
    {
        uri.replace(0, 12, "DIXI:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DIXI.lnk";
}
...
{
    return GetAutostartDir() / "DIXI.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DIXI-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DIXI-qt [" + tr("command-line options") + "]                     " + "\n";
